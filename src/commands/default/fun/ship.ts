...
export default {
    ...
    execute: async (interaction: Eris.CommandInteraction) => {
        ...
        if (interaction.data.resolved) {
            interaction.data.resolved.users?.forEach((user) => usernames.push(user.username));
            if (usernames) {
                interaction.createMessage({
                    embeds: [
                        {
                            author: {
                                "name": !usernames[1] ? `We ship ${usernames[0]} with itself. Aww!` : `We ship ${usernames[0]} with ${usernames[1]}. Aww!`,
                            },
                            color: Colors.Pink,
                        },
                    ],
