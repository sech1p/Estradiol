  ...
  execute: async (interaction: Eris.CommandInteraction) => {
    ...
    axios
      ...
      .then((response) => {
        ...
        response.data.names.forEach(
          (name: any) =>
            (names += `${name.value} [Status: ${name.status}]
`),
        );
        response.data.pronouns.forEach(
          (pronoun: any) =>
            (pronouns += `${pronoun.pronouns} [Status: ${pronoun.status}]
`),
        );
        response.data.flags.forEach(
          (flag: any) =>
            (flags += `${flag.name} (${flag.description ?? "No description has been provided"})
`),
        );
