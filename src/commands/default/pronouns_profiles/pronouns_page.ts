  ...
  execute: async (interaction: Eris.CommandInteraction) => {
    ...
    axios
      ...
      .then((response) => {
            ...
            embeds: [
              {
                ...
                description:
                  "Failed to fetch information from API or user does not exists",
                color: Colors.Red,
              },
            ],
          });
        ...
        response.data.profiles.en.names.forEach(
          (name: any) =>
            (names += `[${name.opinion}] ${name.value}
`),
        );
        response.data.profiles.en.pronouns.forEach(
          (pronoun: any) =>
            (pronouns += `[${pronoun.opinion}] ${pronoun.value}
`),
        );
        response.data.profiles.en.flags.forEach(
          (flag: any) =>
            (flags += `${flag}
`),
        );
